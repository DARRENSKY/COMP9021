...
'''
A class Savings and a class Loans that both derive from a class
Account, allowing one to set the values of at most N-1 parameters
from a list of N parameters, and in case the values of N-1 parameters
are set, computing the value of the missing one.
'''
...
class Account:
    ...
    # - interest and reference_period have to be provided and their values cannot be changed.
    ...
    # - final_sum for savings (either provided or computed),
    #   useless for loans since it has to be 0 (loan fully repaid)
    # - initial_sum for loans (either provided or computed),
    #   useless for savings since it is the same as term_amount.
    def __init__(self, *, interest, reference_period, term_amount, duration):
        # We will remove INITIAL_SUM from _unknowns when dealing with an object
        # of class Savings, and remove FINAL_SUM from _unknowns when dealing
        # with an object of class Loan.
        self._unknowns = {Account.INITIAL_SUM, Account.TERM_AMOUNT, Account.FINAL_SUM,
                                                                                    Account.DURATION
                          }
    ...
    def set_term_amount(self, term_amount):
        # An amount added to Savings account, and deducted from a Loans account.
    ...
    def set_reference_period(self, reference_period):
        if reference_period not in Account.nb_in_year:
            raise ValueError(f'Reference period should be one of {list(Account.nb_in_year)}')
    ...
    def set_effective_interest(self):
        self.effective_interest = ((1 + self.interest / Account.nb_in_year[self.reference_period])
                                                    ** Account.nb_in_year[self.reference_period] - 1
                                  )
    ...
    def check_and_set_parameter(self, parameter, parameter_name, valid_types, sign):
    ...
    def check_type(self, parameter, parameter_name, valid_type):
        if not isinstance(parameter, valid_type):
            raise TypeError(f'{parameter_name} should be of type in {valid_type}')
        ...
        if parameter:
            if parameter_name in self._unknowns:
                if len(self._unknowns) == 1:
                    raise NoUnknownException(f'{parameter_name} is the only unknown parameter')
...
class Savings(Account):
    ...
    def __init__(self, *, interest, reference_period = 'year', term_amount = 0, duration = 0,
                                                                                       final_sum = 0
                ):
    ...
    def solve(self):
        ...
        if Account.FINAL_SUM in self._unknowns:
            ...
            self._duration = log10(self.final_sum * self.effective_interest /
                                                    self.term_amount + (1 + self.effective_interest)
...
class Loan(Account):
    ...
    def solve(self):
        ...
        if Account.INITIAL_SUM in self._unknowns:
            self._initial_sum = -self.term_amount * ((1 + monthly_interest) **\
                                                                            (12 * self.duration) - 1
                                                    ) / monthly_interest /\
                                                      (1 + monthly_interest) ** (12 * self.duration)
        elif Account.TERM_AMOUNT in self._unknowns:
            self._term_amount = -self.initial_sum * (1 + monthly_interest) **\
                                                     (12 * self.duration) * monthly_interest /\
                                                ((1 + monthly_interest) ** (12 * self.duration) - 1)
        else:
            self._duration = log10(self.term_amount / (monthly_interest * self.initial_sum +
                                                                                   self.term_amount)
