...
class QueueSimulation:
    ...
    def compute_and_display_estimates(self):
        ...
        average_nb_in_queue = traffic_intensity ** 2 / (1 - traffic_intensity)
        average_waiting_time_in_queue = average_nb_in_queue * self.average_time_between_two_arrivals
        print('Estimated average number of customers in queue including those being served: '
                 f'{round(average_nb_in_queue + traffic_intensity, 2)}'
             )
        print('Estimated average number of customers in queue waiting to be '
                                f'served: {round(average_nb_in_queue, 2)}'
             )
        print('Estimated average waiting time, excluding serving time:', end = '')
        self.display_time(average_waiting_time_in_queue)
        print('Estimated average waiting time, including serving time:', end = '')
        self.display_time(average_waiting_time_in_queue + self.average_service_time)
        print()
